...
# note: read_stars for a .tif file
...
# because the order matters, if I had included world_cities first, it would have plotted UNDER the world polygons
```
...
tm_shape(world_elevation) +                      # raster data, plotting global elevations
  tm_raster() +
tm_shape(world_vector) + 
  tm_borders() +                                 # borders, so polygons don't have fill
tm_shape(world_cities) +
  tm_dots() + 
  tm_text("name", size = 0.7, col = "blue" )     # adding city text on top by specifying column name
...
tm_shape(world_cities) +  # defines spatial extent and CRS
  tm_dots() + 
  tm_text("names") +
...
# everything is plotted on top of world cities!
```
...
```{r}
# controlling spatial extent independent of plotting order
...
```{r}
# setting bounding box using argument in tm_shape

tm_shape(world_elevation, bbox = c(-15, 35, 45, 65)) + # vector contains long/lat values (W, S, E, N) or (xmin, ymin, xmax, ymax)
  tm_raster() +
tm_shape(world_vector) +
  tm_borders()
...
```{r}
# can restrict bounding box to a dataset rather than a specific vector

tm_shape(world_elevation, bbox = world_cities) + # want it to be the same extent as world_cities is
  tm_raster() +
tm_shape(world_vector) +
  tm_borders()
```
...
```{r}
# modifying aesthetics within tm_polygons()

tm_shape(world_vector) +
  tm_polygons(fill = "lightblue",
              col = "blue4",
              lwd = 0.5, # width of border
...
```{r}
# plotting world cities as symbols, no background map
...
```{r}
# trying different symbols

tm_shape(world_cities) +
  tm_symbols(shape = 3) # use numbers to specify shape!
  #tm_bubbles()
...
```{r}
# updating legend specifications, like: fill, color scale, title, and position

tm_shape(world_vector) +
  tm_polygons(fill = "life_expectancy",
              fill.scale = tm_scale(values = c("yellow", "darkgreen")), # first color: lower, second: upper
              fill.legend = tm_legend(title = "Life Expectancy (years)", position = tm_pos("right")))
...
tm_shape(world_vector) +
  tm_polygons(fill = "life_expectancy",
              fill.scale = tm_scale(values = viridis(8)), # using built-in color-palette
...
tm_shape(world_vector) + 
  tm_polygons(fill = "life_expectancy",
              ...
              #fill.scale = tm_scale(values = "Paster1"),
...
```{r}
# adding symbols, with size dependent on a variable within the dataset

tm_shape(world_vector) +
  tm_polygons(fill = "lightblue") +
tm_shape(world_cities) +
  tm_symbols(
    size = "pop2020", # size of circle dependent on pop2020 variable
    size.legend = tm_legend(orientation = "portrait") # other orientation option is "landscape"
  )
...
tm_shape(world_vector) + 
  tm_fill() +
  tm_grid() # adds grid on top of the map
```
...
```{r}
# adding compass and scalebar

tm_shape(world_vector) +
  tm_fill() +
  tm_scalebar(position = c("left", "top")) + # can specify position of both!
  tm_compass(position = c("left", "top"), north = 180) # north specification decides direction compass points in
...
tm_shape(world_vector) +
  tm_polygons(fill = "wb_income_region", # fill by income
              fill.legend = tm_legend(title = "Regional Income")) + # legend title
  tm_title(text = "Global Income", 
           frame = TRUE) + # puts a box around the title
  tm_layout(text.fontfamily = "Arial", # change font of text
            style = "beaver", # color scheme
            bg.color = "grey95", # background color
            color_vision_deficiency_sim = ("protan"),
            frame.color = "blue",
            frame.double_line = TRUE) + # double frame!
  tm_crs("+proj=eqearth")
```


```{r}
# specifying bins of legend (but doesn't actually work for me)
...
```{r}
# running this line will change the mode of R, meaning that all the maps we make now will be interactive
tmap_mode("view")
```
...
tmap_save(map1, here::here("week1", "tmap_example.png")) # last piece is always just what you want to call the file
```
